...
def step_impl(context):
    assert_equal(context.search_results_page.get_page_title(),
                 "Search results · PyPI")


@step('I see a search result "{search_result}"')
def step_impl(context, search_result):
    package_name, package_version = search_result.split(" ")
    assert_true(context.search_results_page.find_search_result(package_name, package_version))
